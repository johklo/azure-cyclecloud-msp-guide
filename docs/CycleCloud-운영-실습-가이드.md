...
> **스크립트만 바꿀 때** — 같은 파일명의 내용만 고치면 이미 실행된 노드에서는 건너뛰므로(`.run` 마커가 파일명 단위) 노드 재생성이 필요하다.
>
> 자세한 규칙은 [4. Cluster-Init 및 커스텀 스크립트](04-cluster-init-및-커스텀-스크립트.md) 참고.
...
> **참고** — `[[[volume]]]` 옵션: `Size`(GB), `SSD=True`(또는 `StorageAccountType=Premium_LRS`), `fs_type`(ext4/xfs). 기존 디스크 재사용은 `VolumeId=<disk-id>`. `[[[volume]]]` 은 scaleset 속성이라 반영하려면 nodearray 전체 재생성이 필요하다(위 ⚠️). 템플릿 조각: [`../templates/add-disk.txt`](../templates/add-disk.txt)
>
> ⚠️ `Persistent=true`(노드 종료 후에도 유지)는 **단일 노드(`[[node ...]]`) 전용**이다. 위처럼 `nodearraybase` 에 넣으면 이를 상속하는 것이 전부 nodearray(=VMSS)라 `Scaleset disk 'datadisk' cannot be persistent` 로 노드 기동이 실패한다. 계산 노드에서 데이터를 유지해야 하면 디스크가 아니라 §7A 의 NFS 를 쓴다. 자세한 구분은 [7. 데이터 디스크 마운트](07-데이터-디스크-마운트.md) 참고.
