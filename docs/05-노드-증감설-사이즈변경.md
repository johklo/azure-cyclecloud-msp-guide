...
동작 중인 VM 크기는 실시간으로 변경되지 않는다. **SKU를 변경한 뒤 기존 노드를 Terminate하고 새 크기로 재생성**하는 순서로 진행한다.
...
- SKU 변경 후 `sinfo -N -o '%N %T %c %m'` 로 노드가 `inval` 이 아니고 CPU/메모리 값이 새 SKU 와 일치하는지 확인한다.
